  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-slate-800 bg-slate-900/95 backdrop-blur-md text-white" id="app-header">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        {/* Desktop Navigation */}
        <nav className="hidden md:ml-6 md:flex md:items-center md:gap-x-4 lg:gap-x-6">
          {menuItems.map((item) => (
            <button
              key={item.id}
              onClick={() => handleNavClick(item.id)}
              className="text-xs lg:text-sm font-semibold uppercase tracking-wider text-slate-300 transition-colors hover:text-white"
            >
              {item.label}
            </button>
          ))}
        ...
        <div className="hidden md:flex items-center gap-x-3 lg:gap-x-4">
          <a
            ...
            className="hidden lg:flex items-center gap-1.5 text-[11px] lg:text-xs text-slate-400 hover:text-white font-mono transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-full bg-white px-4 py-2 text-xs font-semibold text-slate-900 shadow-md hover:bg-slate-100 active:scale-95 transition-all cursor-pointer"
          >
            Get Quote
